...
var enoughMoneyToPay = () =>
  parseFloat(document.getElementById("amountToPay").innerText) <
  parseFloat(document.getElementById("priceToPay").innerText);
...
var calculateCoinsToReturn = (coinsList, amount) => {
  ...
  for (coin in coinsList) {
    ...
    if (coinValue <= pendingAmount) {
      ...
      var coinQuantity = {
        coin: coin,
        quantity:
          quantity <= coinsList[coin].stock ? quantity : parseInt(coinsList[coin].stock),
      };
  ...
  if (pendingAmount > 0) {
    returnedStatus.changeIsPossible = false;
    returnedStatus.coinsToReturnList = [];
  } else {
    returnedStatus.changeIsPossible = true;
    returnedStatus.coinsToReturnList = coinsToReturnList;
  }
...
var updateCoinsStock = (returnedCoins, coinsList) => {
  ...
  for (returnedCoin of returnedCoins) {
    coinsList[returnedCoin.coin].stock = String(
      parseInt(coinsList[returnedCoin.coin].stock) - parseInt(returnedCoin.quantity)
    );
  }
  var i = 0;
  for (coin in coinsList) {
    coinsList[coin].stock = String(
      parseInt(coinsList[coin].stock) + parseInt(coinsList[coin].unitsToPay)
    );
...
var handlePayButtonClick = coinsList => {
  ...
  if (coinsToReturn.changeIsPossible) {
    updateCoinsStock(coinsToReturn.coinsToReturnList, coinsList);
    amountToReturnField.innerText = "Billetes y monedas a devolver => " + returnAmount + "€";
    displayCoinsToReturn(coinsToReturn.coinsToReturnList, coinsList);
  } else {
    amountToReturnField.innerText = "No se puede dar el cambio => " + returnAmount + "€";
  }
};
...
var createCoinQuantity = (coinsList, coin) => {
  ...
  quantityInput.addEventListener("change", event => {
    inputChangeEvent(event, coinsList, coin);
  });
...
document.getElementById("price-button").addEventListener("click", () => {
  handlePriceButtonClick(coins);
});
document.getElementById("pay-button").addEventListener("click", () => {
  handlePayButtonClick(coins);
});
...
var irpfSlots = [
  ...
  { min: 20200, max: 35200, percentage: 0.3 },
  { min: 35200, max: 60000, percentage: 0.37 },
  { min: 60000, max: 10000000, percentage: 0.45 },
];

var calculateSocialSecurity = grossSalary =>
  (SOCIAL_SECURITY_PERCENTAGE *
    (grossSalary <= SOCIAL_SECURITY_MAX_AMOUNT ? grossSalary : SOCIAL_SECURITY_MAX_AMOUNT)) /
  100;
...
var generateSalaryReport = (grossSalaryAmount, socialSecurityAmount, irpfAmount) => {
  ...
  lineContainer.appendChild(
    generateReportLine("Cantidad destinada a Seguridad Social: ", socialSecurityAmount)
  );
  ...
  lineContainer.appendChild(
    generateReportLine("Salario Neto Mensual (12 pagas): ", (netSalary / 12).toFixed(2))
  );
  lineContainer.appendChild(
    generateReportLine("Salario Neto Mensual (14 pagas): ", (netSalary / 14).toFixed(2))
  );
};

var handleSalaryButtonClick = () => {
  var grossSalaryField = parseFloat(
    document.getElementById("GrossSalaryField").value
  ).toFixed(2);
  if (isNaN(grossSalaryField) || grossSalaryField <= 0) {
    ...
    irpf = calculateIrpf(
      grossSalaryField - socialSecurity - GASTOS_GENERALES_DEDUCIBLES
    ).toFixed(2);
